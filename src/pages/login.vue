<!-- 组件说明 -->
<template>
  <div class="login">
    <div class="container">
      <a href="/index">
        <img src="/imgs/login-logo.png" alt="login-head" />
      </a>
    </div>
    <div class="wrapper">
      <div class="container">
        <div class="login-form">
          <h3>
            <span class="checked">账号登录</span>
            <span>|</span>
            <span>扫码登录</span>
          </h3>
          <div class="input">
            <input type="text" placeholder="请输入账号" v-model="username"/>
          </div>
          <div class="input">
            <input type="text" placeholder="请输入密码" v-model="password"/>
          </div>
          <div class="btn-box">
            <a href="javascript:;" class="btn" @click="login">登录</a>
          </div>
          <div class="tips">
            <div class="sms" @click="register">手机短信登录/注册</div>
            <div class="register">
              <span>立即注册</span>
              <span>|</span>
              <span>忘记密码？</span>
            </div>
          </div>
        </div>
      </div>
    </div>
    <div class="footer">
      <div class="footer-link">
        <a href="https://www.imooc.com/u/1343480" target="_blank">河畔一角主页</a>
        <span>|</span>
        <a href="https://coding.imooc.com/class/113.html" target="_blank">Vue全栈课程</a>
        <span>|</span>
        <a href="https://coding.imooc.com/class/236.html" target="_blank">React全家桶课程</a>
        <span>|</span>
        <a
          href="https://coding.imooc.com/class/343.html"
          target="_blank"
        >微信支付专项课程（H5+小程序/云+Node+MongoDB）</a>
      </div>
      <p class="copyright">Copyright ©2019 mi.futurefe.com All Rights Reserved.</p>
    </div>
  </div>
</template>

<script>
import { SAVE_USERNAME } from "../store/types";

export default {
  components: {},
  data() {
    return {
        username: '',
        password: '',
        userId: ''
    };
  },
  computed: {},
  methods: {
      login() {
          let { username, password } = this;
          this.axios.post("/user/login", {
              username,
              password
          }).then((res)=>{
              this.userId = res.id;
              this.$cookie.set("userId", res.id, "1m");
              this.$store.dispatch(SAVE_USERNAME, res.username);
              this.$router.push("/index");
          })
      },
      register() {
          this.axios.post("/user/register", {
              username: "zhimin",
              password: "zhimin",
              emial: "1023@qq.com"
          }).then(()=>{
              alert("注册成功！");
          })
      }
  }
};
</script>

<style scoped lang="scss">
.login {
  & > .container {
    height: 113px;
    a {
      display: inline-block;
      img {
        width: auto;
        height: 100%;
      }
    }
  }
  .wrapper {
    background: url("/imgs/login-bg.jpg") no-repeat center;
    .container {
      height: 576px;
      .login-form {
        width: 410px;
        height: 510px;
        box-sizing: border-box;
        padding-left: 31px;
        padding-right: 31px;
        background-color: #ffffff;
        position: absolute;
        bottom: 29px;
        right: 0px;
        h3 {
          display: flex;
          flex-direction: row;
          justify-content: space-around;
          align-items: center;
          width: 80%;
          height: 23px;
          line-height: 23px;
          font-size: 24px;
          margin: 40px auto 49px;
          .checked {
            color: #ff6600;
          }
        }
        .input {
          display: inline-block;
          width: 348px;
          height: 50px;
          border: 1px solid #e5e5e5;
          margin-bottom: 20px;
          input {
            border: none;
            width: 100%;
            height: 100%;
            padding: 18px;
            box-sizing: border-box;
          }
        }
        .btn-box {
          width: 100%;
          .btn {
            width: 100%;
            text-align: center;
            height: 50px;
            line-height: 50px;
            font-size: 16px;
            margin-top: 10px;
          }
        }
        .tips {
          width: 100%;
          margin-top: 14px;
          display: flex;
          flex-direction: row;
          justify-content: space-between;
          font-size: 14px;
          cursor: pointer;
          .sms {
            color: #ff6600;
          }
          .register {
            color: #999999;
            span {
              margin: 0 7px;
            }
          }
        }
      }
    }
  }
  .footer {
      height: 100px;
      width: 100%;
      color: #999999;
      padding-top: 60px;
      font-size: 16px;
      text-align: center;
      .footer-link {
          a {
              color: #999999;
          }
          span {
              margin: 0 10px;
          }
      }
      p {
          margin-top: 13px;
      }
  }
}
</style>